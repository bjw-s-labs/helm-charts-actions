...
async function run() {
    ...
    try {
        ...
        const chartsToInstall = responseCharts.filter((x) => !repoConfig["excluded-charts-install"].includes(x));
        const chartsToLint = responseCharts.filter((x) => !repoConfig["excluded-charts-lint"].includes(x));
