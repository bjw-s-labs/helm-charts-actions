...
export declare type ParseExternalReferenceSchema<S extends ReferenceSchema, O extends ParseSchemaOptions, A extends string, P extends string | undefined, R extends JSONSchema7 = Omit<S, "$ref">> = A extends keyof O["references"] ? ParseReference<O["references"][A], O, P, R> : O extends {
    rootSchema: IdSchema;
} ? ParseExternalReferenceWithIdSchema<O, A, P, R> : M.Never;
export declare type ParseDomain<R extends string> = S.Join<L.Pop<S.Split<R, "/">>, "/">;
declare type IdSchema = JSONSchema7 & {
    $id: string;
};
export declare type ParseExternalReferenceWithIdSchema<O extends ParseSchemaOptions & {
    rootSchema: IdSchema;
